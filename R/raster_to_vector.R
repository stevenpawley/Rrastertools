...
raster_to_vector <- function(rasterlayer, gapp = NULL, rmarea_km2,
  ...
  rgrass7::execGRASS("r.to.vect",
    flags = c("s", "overwrite"), input = "rasterlayer",
    type = "area", output = "rasterlayer"
  )
  rgrass7::execGRASS("v.clean",
    input = "rasterlayer", output = "rasterlayerclean",
  ...
  for (i in 1:cycles) {
    ...
    rgrass7::execGRASS("v.generalize",
      input = invector, output = "rasterlayersimple",
      method = "reduction", threshold = simplify_thres, flags = "overwrite"
    )
    rgrass7::execGRASS("v.generalize",
      input = "rasterlayersimple", output = invector,
  ...
  # remove small areas
  rgrass7::execGRASS("v.clean",
    input = invector, output = "final",
    tool = "rmarea", threshold = rmarea_km2 * 1000000, flag = "overwrite"
  )

  generalized_py <- rgrass7::readVECT(vname = "final")
  generalized_py <- sf::st_as_sf(generalized_py)
